...
/** A jewellery shape (ring, necklace, …). Shapes are user-defined. */
export type JewelryShape = string;
...
/**
 * A metal family the shop trades in — Or, Argent, or any other material the
 * user creates (Platine, Acier, Plaqué Or…). Every stock type points at one,
 * which is what makes the app metal-agnostic instead of silver-only.
 */
export interface MetalCategory {
  id: string;
  name: string;       // French label, e.g. "Or"
  nameAr: string;     // Arabic label, e.g. "ذهب"
  color: string;      // accent colour used across the UI
  calibres: string[]; // purities valid for this metal, e.g. ['18k','21k','24k']
  /** Reference buy price per gram (DZD) — used for margin calculations. */
  pricePerGram: number;
  /** Built-in categories (or / argent) cannot be deleted, only edited. */
  isBuiltIn?: boolean;
}

/**
 * A stock type: a concrete article family the shop holds, e.g. "Or 18k Italien"
 * or "Argent 925 Local". Formerly `MetalType`.
 */
export interface MetalType {
  id: string;
  name: string;
  /** Which metal family this type belongs to — see MetalCategory. */
  metalCategoryId: string;
  calibre: string;
  ...
  shapes: Record<JewelryShape, number>; // weight in grams (or piece count if isAlaPiece)
}
...
export interface Command {
  ...
  shape?: JewelryShape; // for industry
  paidAmount: number;
