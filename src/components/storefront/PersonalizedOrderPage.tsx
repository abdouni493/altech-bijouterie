...
const PersonalizedOrderPage: React.FC<PersonalizedOrderPageProps> = ({ lang, setCurrentPage, setLastOrderId, theme = 'light' }) => {
  const { metalTypes, webDeliveryCompanies, addWebOrder, categoryLabel } = useApp();
  ...
  const [form, setForm] = useState({
    ...
    metalTypeId: '', shape: '', maxWeight: '', notes: '',
  });
  ...
  const t = {
    fr: {
      ...
      metalType: "Type de métal", shape: 'Forme / Modèle', maxWeight: 'Poids maximum souhaité (grammes)',
      ...
      selectMetal: "Type de métal", selectWilaya: 'Sélectionner votre wilaya',
    ...
    ar: {
      ...
      metalType: 'نوع المعدن', shape: 'الشكل / النموذج', maxWeight: 'الوزن الأقصى المطلوب (غرام)',
      ...
      selectMetal: 'نوع المعدن', selectWilaya: 'اختر ولايتك',
  ...
  const selectedMetalType = metalTypes.find(s => s.id === form.metalTypeId);
  const shapes = selectedMetalType ? Object.keys(selectedMetalType.shapes) : [];
  ...
  const handleSubmit = async () => {
    ...
    const newId = await addWebOrder({
      ...
      personalizedDetails: {
        metalType: selectedMetalType?.name || '', calibre: '', form: form.shape,
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          >
            ...
            <SelectWrap fieldKey="metalTypeId">
              <label style={labelStyle(isDark)}>{t.metalType}</label>
              <select
                value={form.metalTypeId}
                onChange={e => setForm(f => ({ ...f, metalTypeId: e.target.value, shape: '' }))}
                style={{ ...inputStyle(false, isDark), paddingRight: 36, appearance: 'none' }}
                onFocus={focusOn} onBlur={e => focusOff(e, 'metalTypeId')}
              >
                <option value="">{t.selectMetal}</option>
                {metalTypes.map(s => <option key={s.id} value={s.id}>{categoryLabel(s.id)} · {s.name}</option>)}
              </select>
            </SelectWrap>
