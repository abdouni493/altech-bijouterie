...
const DEFAULT_MARQUEE = ['💎 Or & Argent Certifiés', 'Livraison nationale', 'Créations sur mesure', 'Bijoux artisanaux', 'Qualité garantie'];
              ...
              >
                ...
                <div style={{ padding: '16px 18px', background: tc.cardBg }}>
                  ...
                  {offer?.unitPrice
                    ...
                    : <div style={{ fontFamily: MESSIKA_FONTS.body, fontSize: '11px', color: tc.textMuted }}>
                        {[categoryLabel(offer?.metalTypeId), offer?.calibre].filter(Boolean).join(' ') || '—'}
                      </div>}
                </div>
...
const LandingPage: React.FC<LandingPageProps> = ({ setCurrentPage, lang, theme = 'dark' }) => {
  ...
  const t = {
    fr: {
      ...
      heroLine2: settings.webHeroLine2Fr || 'Or & Argent',
      heroDesc: settings.webHeroDescFr || 'Découvrez nos collections en or et en argent massif, conçues pour celles et ceux qui apprécient la qualité et l\'artisanat authentique.',
      ...
      storyDesc: settings.webStoryDescFr || 'Depuis plus d\'une décennie, chaque pièce est façonnée avec passion et expertise — or massif comme argent 925 certifié — pour sublimer votre élégance naturelle.',
      ...
      stat2: settings.webStat2LabelFr || 'Or & Argent certifiés',
      stat3: settings.webStat3LabelFr || 'Note clients',
      stat4: settings.webStat4LabelFr || 'Délai livraison',
      trust1: settings.webBenefit1TitleFr || 'Métaux Certifiés',
