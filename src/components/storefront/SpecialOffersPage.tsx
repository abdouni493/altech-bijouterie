...
}> = ({ offer, lang, isAdded, onAddToCart, onViewDetails, onOrderNow, delay, isDark, tc, metalTypeName }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '20px' }}>
        ...
        {(metalTypeName || offer.calibre || offer.form || (offer.showWeight && offer.weight)) && (
          <div style={{ fontFamily: MESSIKA_FONTS.body, fontSize: '11px', color: tc.textMuted, marginBottom: '14px', letterSpacing: '0.08em' }}>
            {[metalTypeName, offer.calibre, offer.form, offer.showWeight && offer.weight ? `${offer.weight}g` : null].filter(Boolean).join(' — ')}
          </div>
        )}
...
const SpecialOffersPage: React.FC<SpecialOffersPageProps> = ({ lang, cart, setCart, setCurrentPage, theme = 'dark' }) => {
  const { webSpecialOffers, metalTypes } = useApp();
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2px' }} className="special-grid">
            {activeOffers.map((offer, i) => (
              <PromoCard
                ...
                isDark={isDark}
                tc={tc}
                metalTypeName={metalTypes.find(s => s.id === offer.metalTypeId)?.name}
              />
            ))}
          </div>
        )}
