...
const OffersPage: React.FC<OffersPageProps> = ({ lang, cart, setCart, setCurrentPage, theme = 'dark' }) => {
  const { webOffers, metalTypes } = useApp();
              ...
              {filtered.map((offer, idx) => (
                ...
                >
                  <ProductCard
                    ...
                    title={offer.name}
                    subtitle={(() => {
                      const metalTypeLabel = metalTypes.find(s => s.id === offer.metalTypeId)?.name || '';
                      const weightPart = offer.showWeight && offer.weight ? `${offer.weight}g` : '';
                      return [weightPart, metalTypeLabel].filter(Boolean).join(' — ') || undefined;
                    })()}
