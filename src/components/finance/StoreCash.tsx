...
const StoreCash: React.FC = () => {
  const {
    ...
    workshops, commands, workers, workerPayments, clients, language, metalPriceFor, categoryOf
  } = useApp();
  ...
  const [showMetalDetail, setShowMetalDetail] = useState(false);
  ...
  const filteredDeliveries = getFilteredData(deliveries.map(d => ({ ...d, date: d.createdDate })));
  const filteredCommands = getFilteredData(commands);
  ...
  const totalExpenses = filteredStoreExpenses.reduce((sum, exp) => sum + (exp.price || 0), 0);

  const totalDeliveryCost = filteredDeliveries.reduce((sum, delivery) => {
    return sum + (delivery.paymentHistory?.reduce((paySum: number, p: any) => paySum + (p.amount || 0), 0) || 0);
  }, 0);

  // What the workshops were paid over the period, taken from finalized orders.
  const totalWorkplaceCost = filteredCommands.reduce((sum, cmd) => sum + (Number(cmd.workshopPrice) || 0), 0);
  ...
  // Cost basis of what was sold, valued with each metal category's own
  // reference price (gold at the gold price, silver at the silver price…).
  const metalBreakdown = (() => {
    const rows = new Map<string, { name: string; color: string; weight: number; cost: number; revenue: number; pricePerGram: number }>();
    filteredSales.forEach(sale => {
      (sale.items || []).forEach((item: any) => {
        if (item.isAlaPiece) return;                    // pieces have no weight basis
        const cat = categoryOf(item.metalTypeId);
        if (!cat) return;
        const weight = Number(item.weight) || 0;
        const row = rows.get(cat.id) || { name: cat.name, color: cat.color, weight: 0, cost: 0, revenue: 0, pricePerGram: cat.pricePerGram };
        row.weight += weight;
        row.cost += weight * metalPriceFor(item.metalTypeId);
  ...
  const metalCost = metalBreakdown.reduce((s, r) => s + r.cost, 0);
  const metalRevenue = metalBreakdown.reduce((s, r) => s + r.revenue, 0);
  const metalMarginBenefit = metalRevenue - metalCost;
  ...
  return (
    ...
    >
      ...
      {metalBreakdown.length > 0 && (
        <div className="lux-card" style={{ overflow: 'hidden' }}>
          <button
            onClick={() => setShowMetalDetail(!showMetalDetail)}
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ textAlign: 'left' }}>
                <p style={{ fontSize: 14, fontWeight: 800, color: 'var(--silver-100)', margin: 0 }}>Marge par Métal</p>
                <p style={{ fontSize: 10, color: 'var(--silver-300)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '2px 0 0' }}>
                  {metalBreakdown.map(r => `${r.name} ${r.pricePerGram.toLocaleString()} DZD/g`).join('  ·  ')}
                </p>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontSize: 18, fontWeight: 800, color: metalMarginBenefit >= 0 ? 'var(--gold)' : 'var(--danger)' }}>
                {Math.round(metalMarginBenefit).toLocaleString()} DZD
              </span>
              {showMetalDetail ? <ChevronUp size={18} style={{ color: 'var(--silver-300)' }} /> : <ChevronDown size={18} style={{ color: 'var(--silver-300)' }} />}
            </div>
          </button>
          ...
          <AnimatePresence>
            {showMetalDetail && (
              ...
              >
                <div style={{ padding: '0 24px 24px', borderTop: '1px solid var(--border)', paddingTop: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {metalBreakdown.map(row => {
                    const margin = row.revenue - row.cost;
                    return (
                      <div key={row.name} style={{ padding: '14px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                          <span style={{ width: 10, height: 10, borderRadius: '50%', background: row.color, flexShrink: 0 }} />
                          <p style={{ fontSize: 13, fontWeight: 800, color: 'var(--silver-100)', margin: 0 }}>{row.name}</p>
                          <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--silver-400)' }}>{row.pricePerGram.toLocaleString()} DZD/g</span>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10 }}>
                          <div>
                            <p style={{ fontSize: 9, fontWeight: 700, color: 'var(--silver-400)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 4px' }}>Poids vendu</p>
                            <p style={{ fontSize: 14, fontWeight: 800, color: 'var(--silver-100)', margin: 0 }}>{row.weight.toFixed(2)} g</p>
                          </div>
                          <div>
                            <p style={{ fontSize: 9, fontWeight: 700, color: 'var(--silver-400)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 4px' }}>Chiffre d'affaires</p>
                            <p style={{ fontSize: 14, fontWeight: 800, color: 'var(--silver-100)', margin: 0 }}>{Math.round(row.revenue).toLocaleString()} DZD</p>
                          </div>
                          <div>
                            <p style={{ fontSize: 9, fontWeight: 700, color: 'var(--silver-400)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 4px' }}>Coût matière</p>
                            <p style={{ fontSize: 14, fontWeight: 800, color: 'var(--silver-100)', margin: 0 }}>{Math.round(row.cost).toLocaleString()} DZD</p>
                          </div>
                          <div>
                            <p style={{ fontSize: 9, fontWeight: 700, color: 'var(--silver-400)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 4px' }}>Marge</p>
                            <p style={{ fontSize: 14, fontWeight: 800, color: margin >= 0 ? 'var(--gold)' : 'var(--danger)', margin: 0 }}>{Math.round(margin).toLocaleString()} DZD</p>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  <p style={{ fontSize: 11, color: 'var(--silver-400)', margin: 0 }}>
                    Chaque métal est valorisé à son propre prix de référence, défini dans le Catalogue.
                  </p>
                </div>
              </motion.div>
            )}
