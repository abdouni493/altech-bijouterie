...
const Dashboard: React.FC = () => {
  ...
  // Daily reference price per metal category, keyed by category id.
  ...
  const stats = useMemo(() => {
    ...
    const totalInventory = metalTypes.reduce((acc, st) => {
  ...
  }, [metalTypes, sales, purchases, workerPayments, commands, workers, workerAdvances, workerAbsences, isAdmin, user, shapes]);
  ...
  const kpis = isAdmin ? [
    { label: 'Stock Total', value: `${stats.totalInventory.toFixed(1)}g`, sub: `${metalTypes.length} type(s)`, icon: Layers, iconBg: 'var(--platinum-100)', iconColor: 'var(--silver-200)', accent: kpiAccents[0] },
      ...
      {isAdmin && (
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <motion.button
              onClick={() => {
                setPriceInputs(Object.fromEntries(metalCategories.map(c => [c.id, String(c.pricePerGram)])));
              ...
              style={{ height: 44, borderRadius: 12, padding: '0 24px', display: 'flex', alignItems: 'center', gap: 10, fontSize: 13, cursor: 'pointer' }}
            >
              <Scale size={16} /> Prix des Métaux / g
            </motion.button>
          </div>
        </div>
      )}
        ...
        <motion.div initial={shouldReduce ? false : { opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="silver-card" style={{ padding: 28 }}>
          ...
          <div className="space-y-4">
            {metalTypes.map(st => {
              ...
              const pct = min > 0 ? Math.min(100, (total / min) * 100) : Math.min(100, (total / (stats.totalInventory || 1)) * 100 * metalTypes.length);
            ...
            {metalTypes.length === 0 && (
              <div style={{ textAlign: 'center', padding: '30px 0' }}>
                ...
                <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--silver-300)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>Aucun type de métal</p>
              </div>
            )}
          </div>
        </motion.div>
          ...
          >
            ...
            >
              <div className="modal-header">
                <div>
                  <div className="silver-line" style={{ marginBottom: 6 }} />
                  <h2 style={{ fontSize: 20, fontWeight: 800, color: 'var(--silver-100)', letterSpacing: '-0.03em', margin: 0 }}>Prix des Métaux / Gramme</h2>
                  <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--silver-300)', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '4px 0 0' }}>Taux de référence journalier par métal</p>
                </div>
                <button onClick={closeShowPriceModal} className="btn-icon" style={{ flexShrink: 0 }}><X size={18} /></button>
              </div>
              <form onSubmit={(e) => {
                e.preventDefault();
                metalCategories.forEach(c => {
                  const v = parseFloat(priceInputs[c.id]);
                  if (!isNaN(v) && v >= 0 && v !== c.pricePerGram) updateMetalCategory(c.id, { pricePerGram: v });
                });
                closeShowPriceModal();
              }}>
                <div className="modal-body space-y-4">
                  <p style={{ fontSize: 12, color: 'var(--silver-400)', margin: 0 }}>
                    Ces prix servent de base au calcul des marges (or, argent et tout autre métal du catalogue).
                  </p>
                  {metalCategories.map(c => (
                    <div key={c.id}>
                      <label className="lux-label" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
