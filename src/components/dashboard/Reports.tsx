...
const Reports: React.FC = () => {
  const {
    sales, purchases, workerPayments, metalTypes, suppliers, workshops,
    ...
    metalPriceFor, categoryOf, webOrders, webDeliveryCompanies
  } = useApp();
  ...
  const reportStats = useMemo(() => {
    ...
    const cogs = filteredSales.reduce((s, sale) => s + sale.items.reduce((si, it) => {
      const cpg = costPerGram[it.metalTypeId] ?? (it.pricePerGram * 0.6);
    ...
    // Cost of the metal sold, each line valued at its own category's price.
    const totalMetalCost = filteredSales.reduce((s, sale) => s + sale.items.reduce(
      (si, it: any) => si + (it.isAlaPiece ? 0 : (Number(it.weight) || 0) * metalPriceFor(it.metalTypeId)), 0), 0);
    const metalPriceBenefit = totalMetalCost > 0 ? totalRevenue - totalMetalCost : 0;

    // Same numbers split per metal, for the report breakdown.
    const metalBreakdown = (() => {
      ...
      filteredSales.forEach(sale => (sale.items || []).forEach((it: any) => {
        if (it.isAlaPiece) return;
        const cat = categoryOf(it.metalTypeId);
        if (!cat) return;
        const weight = Number(it.weight) || 0;
        const row = rows.get(cat.id) || { name: cat.name, color: cat.color, weight: 0, cost: 0, revenue: 0, pricePerGram: cat.pricePerGram };
        row.weight += weight;
        row.cost += weight * metalPriceFor(it.metalTypeId);
        row.revenue += Number(it.totalPrice) || 0;
        rows.set(cat.id, row);
      }));
    ...
    const totalCassiePurchaseCost = filteredCassiePurchases.reduce((s, cp) => s + (Number(cp.totalPrice) || 0), 0);
    ...
    return {
      ...
      totalSoldWeight, metalPriceBenefit, totalMetalCost, metalBreakdown,
      filteredReplacements, filteredDebts,
  ...
  }, [reportGenerated, startDate, endDate, sales, purchases, workerPayments, commands, storeExpenses, deliveries, workerAdvances, workerAbsences, replacements, clients, debts, meltings, metalTypes, cassiePurchases]);
    ...
    <div className="animate-page-enter space-y-8">
      ...
      {reportGenerated && reportStats && (
        <div className="space-y-6 animate-fade-scale">
          ...
          {/* Margin per metal */}
          {reportStats.metalBreakdown.length > 0 && (
            <div style={{ background: 'var(--gold-muted)', border: '1px solid #E8C86B', borderRadius: 20, padding: 24 }}>
              <h3 style={{ fontSize: 13, fontWeight: 800, color: 'var(--gold-deep)', letterSpacing: '0.06em', textTransform: 'uppercase', margin: '0 0 16px' }}>
                Marge par métal (prix de référence du catalogue)
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {reportStats.metalBreakdown.map((row: any) => (
                  <div key={row.name} style={{ background: 'var(--bg-surface)', borderRadius: 14, padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                      <span style={{ width: 10, height: 10, borderRadius: '50%', background: row.color }} />
                      <p style={{ fontSize: 13, fontWeight: 800, color: 'var(--gold-deep)', margin: 0 }}>{row.name}</p>
                      <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--gold-deep)', opacity: 0.75 }}>{row.pricePerGram.toLocaleString()} DZD/g</span>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {[
                        { label: 'Poids vendu', value: row.weight.toFixed(2) + ' g' },
                        { label: 'Ventes', value: Math.round(row.revenue).toLocaleString() + ' DZD' },
                        { label: 'Coût matière', value: Math.round(row.cost).toLocaleString() + ' DZD' },
                        { label: 'Marge', value: Math.round(row.revenue - row.cost).toLocaleString() + ' DZD' },
                      ].map(cell => (
                        <div key={cell.label}>
                          <p style={{ fontSize: 9, fontWeight: 800, color: 'var(--gold-deep)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 4px', opacity: 0.8 }}>{cell.label}</p>
                          <p style={{ fontSize: 15, fontWeight: 900, color: 'var(--gold-deep)', margin: 0, letterSpacing: '-0.03em' }}>{cell.value}</p>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="space-y-4">
                          ...
                          {s.items.map((it, idx) => (
                            <div key={idx} style={{ background: 'var(--bg-surface)', borderRadius: 10, padding: '10px 12px' }}>
                              <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--platinum-400)', margin: '0 0 3px', textTransform: 'capitalize' }}>
                                {(it.shape || '—')} — {metalTypes.find(st => st.id === it.metalTypeId)?.name || it.metalTypeId}
                              </p>
                      ...
                      <div key={r.id} style={{ background: 'var(--bg-muted)', borderRadius: 14, padding: '14px 18px' }}>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div style={{ background: 'var(--bg-surface)', borderRadius: 10, padding: '10px 12px' }}>
                            ...
                            <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink-900)', margin: 0 }}>
                              {r.returnedItem.shape || r.returnedItem.metalTypeId} — {r.returnedItem.weight}g × {r.returnedItem.pricePerGram} = {(Number(r.returnedItem.weight || 0) * Number(r.returnedItem.pricePerGram || 0)).toLocaleString()} DZD
                            </p>
                          </div>
                          {r.newItem && (
                            <div style={{ background: 'var(--bg-surface)', borderRadius: 10, padding: '10px 12px' }}>
                              ...
                              <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink-900)', margin: 0 }}>
                                {r.newItem.shape || r.newItem.metalTypeId} — {r.newItem.weight}g × {r.newItem.pricePerGram} = {(Number(r.newItem.weight || 0) * Number(r.newItem.pricePerGram || 0)).toLocaleString()} DZD
                              </p>
                            </div>
                          )}
                        </div>
                      ...
                      {reportStats.filteredMeltings.map((m: any) => (
                        <div key={m.id} style={{ background: 'var(--bg-muted)', borderRadius: 14, padding: '14px 18px' }}>
                          ...
                          <p style={{ fontSize: 12, color: 'var(--platinum-500)', margin: 0 }}>
                            Déposé dans: {(metalTypes.find(s => s.id === m.targetMetalTypeId)?.name) || m.targetMetalTypeId}
                          </p>
                        </div>
                      ))}
                    </div>
            ...
            {activeModules.has('inventaire') && (
              <div style={{ border: '1px solid var(--platinum-100)', borderRadius: 16, overflow: 'hidden' }}>
                <SectionHeader
                  icon={Layers} label="Inventaire Détaillé" count={metalTypes.length}
